...
- **Bidirectional sync** — web actions are written directly to Supabase `literature_data` via `applyActionToLiteratureData()` for instant feedback. The Zotero plugin polls every 15s, applies pending actions locally, then pushes the updated state back via `silentSyncBack()`, keeping both sides in sync without manual re-export. The `pushToSupabase` function now uses `lastSyncedShareSlug` as fallback when `fixedShareSlug` is empty, ensuring sync-back updates the existing record.
- **Collection-aware DOI routing** — DOI items submitted from the To Read section are placed in the To Read Zotero collection; items from the Claimed section go to the Claimed collection. Existing DOI duplicates are also routed correctly.
- **Web-side status field sync** — `undo_claim` and `undo_report` now update `status`/`readingStatus` fields in addition to tags, so items move between buckets immediately on the web page without waiting for a Zotero re-sync.
...
Items also carry a `collectionPathText` ("Parent / Child / Leaf") used purely for subfolder grouping in the display, independent of which workflow bucket they fall into.
...
For the Cloudflare Pages deployment, the web UI calls the worker with `?direct=1` and applies optimistic updates locally (move/remove the card immediately, then confirm with the server), so claim/undo/add feel instant even without Realtime. A periodic refresh reconciles local state with the server.
